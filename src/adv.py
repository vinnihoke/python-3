...
def guess():
    create = random.randrange(1, 5)
    guess = input("🧐 Pick a number between 1 and 5: ")
    if create == int(guess):
        return True
    else:
        return False
...
while True:
    try:
        ...
        availableCommands = ["q - quit", "n - north",
        ...
        if command == "s":
            if player.current_room.name == "Narrow Passage" and player.current_room.s_to == None:
                choice = guess()
                if choice == True:
                    print("\n\n🛑 Cannot advance")
                else:
                    print("\n\n👻 Run for your life and try again!")
